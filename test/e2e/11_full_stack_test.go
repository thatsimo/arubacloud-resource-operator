...
var _ = Describe("11-FullStack", Ordered, func() {
	const (
		projectName     = "aruba-test-full-stack"
		vpcName         = "aruba-test-full-stack"
		sgName          = "aruba-test-full-stack"
		ruleName        = "aruba-test-full-stack"
		subnetName      = "aruba-test-full-stack"
		elasticIPName   = "aruba-test-full-stack"
		bootVolumeName  = "aruba-test-full-stack"
		dataVolumeName  = "aruba-test-full-stack-data"
		keyPairName     = "aruba-test-full-stack"
		cloudServerName = "aruba-test-full-stack"
		namespace       = "default"
	...
	AfterAll(func() {
		By("cleaning up all resources in reverse order")
		...
		cmd = exec.Command("kubectl", "delete", "elasticip", elasticIPName, "-n", namespace, "--ignore-not-found")
		_, _ = utils.Run(cmd)
	...
	It("should create ElasticIP resource", func(ctx SpecContext) {
		...
		cmd := exec.Command("kubectl", "apply", "-f", "-")
		cmd.Stdin = stringReader(elasticIPManifest)
		_, err = utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred())

		By("waiting for ElasticIP to be Created")
		Eventually(func() string {
			cmd := exec.Command("kubectl", "get", "elasticip", elasticIPName, "-n", namespace, "-o", "jsonpath={.status.phase}")
			output, _ := utils.Run(cmd)
			return output
		}, testTimeout, 10*time.Second).Should(Equal("Created"))

		By("verifying ElasticIP has resourceID")
		cmd = exec.Command("kubectl", "get", "elasticip", elasticIPName, "-n", namespace, "-o", "jsonpath={.status.resourceID}")
		output, err := utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred())
		Expect(output).NotTo(BeEmpty())
	}, SpecTimeout(testTimeout))
	...
	It("should create BlockStorage boot volume", func(ctx SpecContext) {
		...
		cmd = exec.Command("kubectl", "get", "blockstorage", bootVolumeName, "-n", namespace, "-o", "jsonpath={.status.volumeID}")
		output, err := utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred())
		Expect(output).NotTo(BeEmpty())
	}, SpecTimeout(testTimeout))
	...
	It("should create CloudServer with full stack", func(ctx SpecContext) {
		cloudServerYAML, err := utils.LoadSampleManifest("arubacloud.com_v1alpha1_cloudserver_full_stack.yaml", map[string]string{
			...
			"__ELASTICIP_NAME__":        elasticIPName,
			"__ELASTICIP_NAMESPACE__":   namespace,
			"__SG_NAME__":               sgName,
			"__SG_NAMESPACE__":          namespace,
		...
		By("creating CloudServer with full stack configuration")
		cmd := exec.Command("kubectl", "apply", "-f", "-")
		...
		By("verifying full stack integration")
		GinkgoWriter.Printf("✓ Full stack test completed successfully\n")
		GinkgoWriter.Printf("  - Project: %s\n", projectName)
		GinkgoWriter.Printf("  - VPC: %s\n", vpcName)
		GinkgoWriter.Printf("  - SecurityGroup: %s\n", sgName)
		GinkgoWriter.Printf("  - SecurityRule: %s\n", ruleName)
		GinkgoWriter.Printf("  - Subnet: %s\n", subnetName)
		GinkgoWriter.Printf("  - ElasticIP: %s\n", elasticIPName)
		GinkgoWriter.Printf("  - Boot Volume: %s\n", bootVolumeName)
		GinkgoWriter.Printf("  - Data Volume: %s\n", dataVolumeName)
		GinkgoWriter.Printf("  - KeyPair: %s\n", keyPairName)
		GinkgoWriter.Printf("  - CloudServer: %s\n", cloudServerName)
	}, SpecTimeout(testTimeout))
})
