...
var _ = Describe("08-ComputeBasic", Ordered, func() {
	const (
		projectName      = "aruba-test-compute-bsc"
		vpcName          = "aruba-test-compute-bsc"
		sgName           = "aruba-test-compute-bsc"
		ruleName         = "aruba-test-compute-bsc"
		subnetName       = "aruba-test-compute-bsc"
		blockStorageName = "aruba-test-compute-bsc"
		keyPairName      = "aruba-test-compute-bsc"
		cloudServerName  = "aruba-test-compute-bsc"
		namespace        = "default"
	...
	It("should create SecurityRule resource", func(ctx SpecContext) {
		...
		cmd := exec.Command("kubectl", "apply", "-f", "-")
		cmd.Stdin = stringReader(ruleManifest)
		_, err = utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should create CloudServer resource", func(ctx SpecContext) {
		...
		cmd := exec.Command("kubectl", "apply", "-f", "-")
		cmd.Stdin = stringReader(cloudServerManifest)
		_, err = utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred())
